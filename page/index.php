  ...
  if ($user_id) {
      $userQuery = $conn->query("SELECT username, email FROM users WHERE user_id = '$user_id' LIMIT 1");
...
<section id="contact" class="bg-gray-50">
  <div class="max-w-7xl mx-auto py-20">
    ...
    <div class="grid md:grid-cols-2 shadow-lg rounded-2xl overflow-hidden">
      <!-- MAP -->
      <div class="w-full h-full md:h-[500px]">
        ...
        class="w-full h-full" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
      </div>

      <!-- CONTACT FORM -->
      <div class="bg-[#1E3A8A] text-white p-10 flex flex-col justify-center h-auto md:h-[500px]">
        ...
        <form id="contactForm" class="flex flex-col gap-4">
          ...
          <div class="flex items-center bg-white text-gray-700 rounded-full px-4 py-3">
            ...
            <input type="tel" id="phone" placeholder="Telephone" class="flex-1 bg-transparent outline-none">
          </div>

          <textarea id="message" placeholder="Type your message..." rows="3" class="w-full rounded-xl px-4 py-3 text-[#333] mt-2 focus:outline-none resize-none" required></textarea>

          <button type="submit" class="mt-4 bg-white text-[#1E3A8A] font-bold rounded-full py-3 transition hover:bg-gray-200">
            SEND YOUR MESSAGE
          </button>
        </form>
      </div>
  ...
  // Filter hotel berdasarkan city
    ...
    // contact us
      ...
      const whatsappNumber = "6285782688916"; // ganti dengan nomor WA kamu
